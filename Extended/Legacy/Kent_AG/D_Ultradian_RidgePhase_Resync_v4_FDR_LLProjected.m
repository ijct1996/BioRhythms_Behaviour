%% ========================================================================
% Script 4 (v4 - ridge-following ultradian phase resynchronisation + BH/FDR + LL projected aftereffect)
% File: Ultradian_RidgePhase_Resync_v4_FDR_LLProjected.m
% ========================================================================
% Purpose
%   This script tests whether validated raw ultradian ridges show phase
%   reorganisation around light-dark transitions.
%
% Conceptual rule
%   - Raw data remain the biological signal.
%   - Selective-HSub is used upstream only to decide which raw ultradian
%     ridge candidates are credible enough to carry forward.
%   - This script uses only validated raw ridge-following phase trajectories.
%
% Expected inputs
%   1) HSubSupported_PeriodMap.mat from Validate_RawVsSelectiveHSub_Periods_v1.m
%      containing validationMap.CarryForward_Periods.
%   2) WP_TS__*.mat files from Behav_wavelet_v12.m containing
%      pkgTS.tables.RidgePhase_Long and, optionally, RidgeTrajectory_Long.
%
% Main outputs
%   Ultradian_RidgePhase_Resync/Ultradian_RidgePhase_Resync_Output.xlsx
%   Ultradian_RidgePhase_Resync/Ultradian_RidgePhase_Resync_Output.mat
%   Ultradian_RidgePhase_Resync/Figures/*.jpg
%
...
% Input filtering
PRIMARY_PHASE = "All";               % Candidate phase used in validation and phase trajectories
REQUIRE_VALID_FLAG = true;            % Require ValidFlag == true in RidgePhase_Long
INCLUDE_HARMONIC_SENSITIVE_12H = true; % Keep UR_12_18 but preserve flag from validation map

% Ultradian bands to include
UR_BANDS = ["UR_1_3", "UR_3_6", "UR_6_9", "UR_9_12", "UR_12_18"];

% Transition-aligned windows
PERI_WINDOW_H     = 6.0;     % Upper cap for phase extraction around transition/control events
BIN_WIDTH_H       = 0.5;     % Binned phase-coherence curve resolution
SUMMARY_WINDOW_H  = 2.0;     % Pre/post window for DeltaR summary
MIN_POINTS_PER_BIN = 10;     % Minimum phase observations for binned circular summary
MIN_POINTS_SUMMARY = 10;     % Minimum phase observations for pre/post summary

% v3 photoperiod safeguards
ADAPTIVE_TRANSITION_WINDOWS = true;  % Cap transition windows so L20/L22 do not cross neighbouring transitions
TRANSITION_WINDOW_FRACTION  = 0.45;  % Use this fraction of the relevant light/dark interval as the event-specific cap
EXCLUDE_INITIAL_DAYS_FROM_RESYNC = false; % Exclude early days after each weekly photoperiod change from transition analysis
MIN_DAY_FOR_STABLE_RESYNC = 0.0;          % Time_days must be >= this value when exclusion is enabled
LL_PHOTOPERIOD_VALUE = 24.0;              % Photoperiod_h >= this is treated as LL/no-transition

% v4 projected LL aftereffect analysis
% LL has no real LD/DL transition. These settings add a secondary analysis
% anchored to the transition phases inherited from the preceding L22 schedule.
DO_LL_PROJECTED_AFTEREFFECT = true;              % Analyse LL relative to former/projected transition phases
LL_PROJECTED_REFERENCE_PHOTOPERIOD_H = 22.0;     % Immediate antecedent photoperiod in the lengthening design
RUN_LL_PROJECTED_DAY_DECAY = true;               % Test whether projected DeltaR changes across LL days
RUN_L22_VS_LL_PROJECTED_COMPARISON = true;       % Compare L22 real transitions with LL projected transitions

% Inferential statistics and multiple testing
% These tests are performed at candidate level, after the Raw-vs-Selective-HSub
% validation gate has already defined carry-forward ultradian ridges.
ALPHA_FDR = 0.05;              % Benjamini-Hochberg false discovery rate threshold
N_PERM_STATS = 10000;          % Sign-flip permutation iterations for candidate-level tests
MIN_CANDIDATES_FOR_TEST = 3;   % Minimum candidate-level observations per inferential test

% Outputs
SAVE_DPI = 600;
FIG_EXT  = '.jpg';
MAX_EXCEL_ROWS_PER_SHEET = 1000000; % Excel hard limit is 1,048,576 including header
...
% Try to infer the handoff folder from validation output location.
% Expected structure: AcrossPhotoperiod_Input/RawVsSelectiveHSub_PeriodValidation/HSubSupported_PeriodMap.mat
...
if isempty(CandidateDeltaR) || height(CandidateDeltaR) == 0
    ...
    Resync_PrimaryStats_BH_FDR = add_bh_fdr(Resync_PrimaryStats, 'PValue_raw', ALPHA_FDR);
    ...
    Resync_RealVsPseudoStats_BH_FDR = add_bh_fdr(Resync_RealVsPseudoStats, 'PValue_raw', ALPHA_FDR);
    ...
    Resync_RidgePeriodStats_BH_FDR = add_bh_fdr(Resync_RidgePeriodStats, 'PValue_raw', ALPHA_FDR);
    ...
    Resync_RidgePowerStats_BH_FDR = add_bh_fdr(Resync_RidgePowerStats, 'PValue_raw', ALPHA_FDR);
end
...
if isempty(LL_ProjectedCandidateDeltaR) || height(LL_ProjectedCandidateDeltaR) == 0
    ...
    LL_ProjectedPrimaryStats_BH_FDR = add_bh_fdr(LL_ProjectedPrimaryStats, 'PValue_raw', ALPHA_FDR);
    ...
    LL_ProjectedRealVsPseudoStats_BH_FDR = add_bh_fdr(LL_ProjectedRealVsPseudoStats, 'PValue_raw', ALPHA_FDR);
    ...
    LL_ProjectedRidgePeriodStats_BH_FDR = add_bh_fdr(LL_ProjectedRidgePeriodStats, 'PValue_raw', ALPHA_FDR);
    ...
    LL_ProjectedRidgePowerStats_BH_FDR = add_bh_fdr(LL_ProjectedRidgePowerStats, 'PValue_raw', ALPHA_FDR);
    ...
    if RUN_LL_PROJECTED_DAY_DECAY
        ...
        LL_ProjectDayDecayStats_BH_FDR = add_bh_fdr(LL_ProjectDayDecayStats, 'PValue_raw', ALPHA_FDR);
    else
        LL_ProjectDayDecayStats = table();
        LL_ProjectDayDecayStats_BH_FDR = table();
    end
    ...
    if RUN_L22_VS_LL_PROJECTED_COMPARISON
        ...
        L22_vs_LLProjectedStats_BH_FDR = add_bh_fdr(L22_vs_LLProjectedStats, 'PValue_raw', ALPHA_FDR);
    else
        L22_vs_LLProjectedStats = table();
        L22_vs_LLProjectedStats_BH_FDR = table();
    end
...
function TL = build_ll_projected_phase_long(P, periWindowH, adaptiveWindows, windowFraction, llPhotoValue, refPhotoperiodH, LOG)
    ...
    events = projected_ll_event_definitions(refPhotoperiodH);
