function out = extended_script7_profiles_run(extHandoffDir, cfg)
%EXTENDED_SCRIPT7_PROFILES_RUN Local phase events + profile outputs (Extended Script 7).
%
%   Ported from legacy G publication profile script into a local function so
%   Script 7 no longer depends on Kent legacy wrappers.

    if nargin < 2 || isempty(cfg)
        cfg = extended_defaults();
    end

    fprintf('\n=== Extended Script 7: Phase events + profiles ===\n');
    fprintf('Plot mode: %s\n', cfg.plotMode);
%% ----------------------------- SETTINGS ---------------------------------
...
BANDS_TO_USE = string(cfg.bands.primaryUR);
...
PERIOD_BIN_WIDTH_H          = 0.25;
PERIOD_HIST_SMOOTH_SPAN     = 3;
PEAK_RELATIVE_HEIGHT_FRAC   = 0.35;
MIN_CANDIDATES_PER_CLUSTER  = 5;
MIN_MICE_PER_CLUSTER        = 3;
FILTER_PERIOD_PADDING_FRAC  = 0.10;
...
% Outputs
SAVE_DPI = get_cfg_nested_(cfg, {'plot','saveDpi'}, 600);
FIG_EXT = get_cfg_nested_(cfg, {'plot','figExt'}, '.png');
...
if nargin < 1 || isempty(extHandoffDir)
    ...
    acrossFolder = char(string(extHandoffDir));
end
...
outputParent = fileparts(acrossFolder);
outRoot = fullfile(outputParent, sprintf('SelectedValidatedUR_%s', char(mode_label_(cfg.plotMode))));
dirs = struct();
dirs.Root       = outRoot;
...
log_line(LOG, 'Plot mode: %s | dpi=%g | ext=%s', string(cfg.plotMode), SAVE_DPI, FIG_EXT);
%% ----------------------------- LOAD VALIDATION MAP -----------------------
...
out = struct();
out.acrossFolder = acrossFolder;
out.rawFolder = rawFolder;
out.outRoot = outRoot;
out.xlsxOut = xlsxOut;
out.matOut = matOut;
out.logPath = logPath;
out.plotMode = string(cfg.plotMode);
out.clusterCount = height(ClusterSummary);
...
function v = get_cfg_nested_(s, path, defaultValue)
    v = defaultValue;
    cur = s;
    for i = 1:numel(path)
        key = path{i};
...
function label = mode_label_(plotMode)
    switch lower(string(plotMode))
        case "publication"
            label = "PublicationProfiles";
        otherwise
            label = "DevelopmentProfiles";
    end
end
